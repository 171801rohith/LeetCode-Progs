...
public class MinNoOfDaysToMakeMBouquets {
    ...
    public int minDays(int[] bloomDay, int m, int k) {
        ...
        for (int bloom: bloomDay) {
            if (bloom > maxBloomDay) maxBloomDay = bloom;
            if (bloom < minBloomDay) minBloomDay = bloom;
        }

        int low = minBloomDay, high = maxBloomDay, mid;

        while (low < high) {
            mid = low + (high - low) / 2;

            int bouquets = canMake(bloomDay, mid, k);
